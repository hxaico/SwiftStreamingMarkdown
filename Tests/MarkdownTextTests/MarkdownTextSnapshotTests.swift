...
final class MarkdownTextSnapshotTests: SnapshotTestCase {
  ...
  // MARK: - Copilot Action Link Tests

  /// Tests that copilot action links (copilot-action:// scheme) are converted to inline attributed text
  func testCopilotActionLinkConversion() async throws {
    let markdown = """
    Click here to [learn more](copilot-action://composer_prefill?text=hello)
    """
    ...
    // Verify the string content is "learn more" (inline text, not an attachment character)
    XCTAssertEqual(convertedString.string, "learn more", "Copilot action link should render as inline text")

    // Verify .link attribute contains a copilot-action:// URL
    ...
    let expectedStyle = NSUnderlineStyle.single.union(.patternDot)
    XCTAssertEqual(underlineStyle, expectedStyle, "Underline style should be single + patternDot")

    // Verify foreground color is set (not the accent link color)
    var foregroundColor: UIColor?
    convertedString.enumerateAttribute(.foregroundColor, in: fullRange, options: []) { value, _, _ in
      if let color = value as? UIColor {
        foregroundColor = color
      }
    }
    XCTAssertNotNil(foregroundColor, "Should have a foreground color set for paragraph text styling")
  }
